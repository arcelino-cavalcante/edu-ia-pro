...
import { invalidateStructureCache, persistNormalizedStructure } from '../../services/structureService';
...
export const StructureManager = ({ structure, setStructure }) => {
    ...
    const handleDuplicate = async (item) => {
        // if (!confirm(`Deseja duplicar...`)) return;
        setLoading(true);
        ...
        try {
            if (currentPath.length === 0) {
                ...
                await persistNormalizedStructure(updatedStructure);
                invalidateStructureCache();
                setStructure(updatedStructure);
            } else {
                alert("A duplicação aninhada requer salvar a árvore inteira. Implementado apenas para raiz nesta demo.");
            }
        } catch (error) {
            console.error(error);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const itemToSave = {
                ...
                materials: materials,
                children: modalMode === 'create' ? [] : (editingItem.children || []),
                ...(nodeType === 'curso' || nodeType === 'trilha' ? { coverImage: coverImageUrl } : {})
            };

            const updateRecursive = (items, pathIdx) => {
                if (pathIdx === currentPath.length) {
                    if (modalMode === 'create') return [...items, itemToSave];
                    return items.map(i => i.id === editingItem.id ? itemToSave : i);
                }
            ...
            await persistNormalizedStructure(newStructure);
            invalidateStructureCache();
    ...
    const confirmDelete = async () => {
        ...
        try {
            if (currentPath.length === 0) {
                ...
                // Delete Child Item (Recursive update)
